
package negocio.ui;

import proyecto.Database;
import proyecto.producto.Producto;

import javax.swing.*;
import javax.swing.table.DefaultTableModel;
import java.awt.*;
import java.sql.*;

public class ProductoPanel extends JPanel {
    private final java.util.function.Supplier<Database> dbSupplier;
    private JTable table;

    public ProductoPanel(java.util.function.Supplier<Database> dbSupplier) {
        this.dbSupplier = dbSupplier;
        setLayout(new BorderLayout());
        table = new JTable();
        add(new JScrollPane(table), BorderLayout.CENTER);
        JPanel south = new JPanel();
        JButton btnLoad = new JButton("Actualizar");
        btnLoad.addActionListener(e -> cargarProductos());
        south.add(btnLoad);
        add(south, BorderLayout.SOUTH);
        cargarProductos();
    }

    private void cargarProductos() {
        Database db = dbSupplier.get();
        DefaultTableModel model = new DefaultTableModel(new Object[]{"ID","Nombre","Precio","Descripcion"},0);
        if (db == null) { table.setModel(model); return; }
        try (Connection conn = db.connect();
             Statement stmt = conn.createStatement();
             ResultSet rs = stmt.executeQuery("SELECT id, nombre, precio, descripcion FROM productos")) {
            while (rs.next()) {
                model.addRow(new Object[]{rs.getInt(1), rs.getString(2), rs.getDouble(3), rs.getString(4)});
            }
            table.setModel(model);
        } catch (Exception e) {
            JOptionPane.showMessageDialog(this, "Error al cargar productos: " + e.getMessage());
            table.setModel(model);
        }
    }
}
